...
def DenseBlock(input, filters, i):
	...
	dense0 = Conv2D(filters, (3, 3), activation = 'relu', padding = 'same')(input)
	dense0 = BatchNormalization()(dense0)

	dense1 = Conv2D(filters, (3, 3), activation = 'relu', padding = 'same')(dense0)
	dense1 = BatchNormalization()(dense1)
	dense1 = concatenate([dense0, dense1], axis = -1)

	dense2 = Conv2D(filters, (3, 3), activation = 'relu', padding = 'same')(dense1)
	dense2 = BatchNormalization()(dense2)
	dense2 = concatenate([dense0, dense1, dense2], axis = -1)

	dense3 = Conv2D(filters, (3, 3), activation = 'relu', padding = 'same')(dense2)
	dense3 = BatchNormalization()(dense3)
	dense3 = concatenate([dense0, dense1, dense2, dense3], axis = -1)

	output = Conv2D(filters, (1, 1), activation = 'relu', padding = 'same')(dense3)
	model = Model(inputs = [input], outputs = [output], name = 'dense_' + str(i))
	return model

def ResidualBlock(input, filters, i, transition = True):
	input = Input(shape=input.shape.as_list()[1:])
	residual = Conv2D(filters, (3, 3), activation = 'relu', padding = 'same')(input)
	...
	if transition:
		output = Conv2D(filters, (1, 1), activation = 'relu', padding = 'same')(input)
...
def SqueezeExcitatioBlock(input, filters, i):
	...
	conv = Conv2D(filters, (3, 3), activation = 'relu', padding = 'same')(se)
	output = SqueezeExcitationModel(conv.shape.as_list()[1:], filters)(conv)
	model = Model(inputs = [input], outputs = [output], name = 'squeeze_' + str(i))
	return model
...
def ConvolutionBlock(input, filters, i):
	...
	output = Conv2D(filters, (3, 3), activation = 'relu', padding = 'same')(conv)
	model = Model(inputs = [input], outputs = [output], name = 'convolution_' + str(i))
	return model

def InceptionBlock(input, filters, i):
	input = Input(shape=input.shape.as_list()[1:])
	inception0 = Conv2D(int(filters/4), (1, 1), activation = 'relu', padding = 'same')(input)
	inception0 = BatchNormalization()(inception0)
	inception0 = Conv2D(int(filters/4), (1, 1), activation = 'relu', padding = 'same')(inception0)

	inception1 = Conv2D(int(filters/4), (1, 1), activation = 'relu', padding = 'same')(input)
	inception1 = BatchNormalization()(inception1)
	inception1 = Conv2D(int(filters/2), (3, 3), activation = 'relu', padding = 'same')(inception1)

	inception2 = Conv2D(int(filters/4), (1, 1), activation = 'relu', padding = 'same')(input)
	inception2 = BatchNormalization()(inception2)
	inception2 = Conv2D(int(filters/8), (3, 3), activation = 'relu', padding = 'same')(inception2)
	inception2 = BatchNormalization()(inception2)
	inception2 = Conv2D(int(filters/8), (3, 3), activation = 'relu', padding = 'same')(inception2)

	inception3 = MaxPooling2D(pool_size = (3, 3), padding = 'same', strides = (1, 1))(input)
	inception3 = Conv2D(int(filters/8), (1, 1), activation = 'relu', padding = 'same')(inception3)
...
def addBlock(input0, input1, filters, i):
	...
	layer1 = Conv2D(filters, (1, 1), activation = 'relu', padding = 'same')(input1)
	output = add([input0, layer1])
...
def concatBlock(input0, input1, filters, i):
	...
	layer1 = Conv2D(filters, (1, 1), activation = 'relu', padding = 'same')(input1)
	output = concatenate([input0, layer1], axis=-1)
...
def attentionBlock(input0, input1, filters, i):
	...
	layer1 = Conv2D(filters, (1, 1, 1), padding = 'same')(input1)
	layer2 = BatchNormalization()(input0)
	layer2 = Conv2D(filters, (1, 1, 1), padding = 'same')(layer2)
	gate = add([layer2, layer1])
	gate = Activation('relu')(gate)
	gate = BatchNormalization()(gate)
	gate = Conv2D(filters, (1, 1, 1), activation = 'sigmoid', padding = 'same')(gate)
...
def UNET(block_type, merge_type, dim = (160, 160, 1), filters = 16):
	...
	output = Conv2D(1, (1, 1), activation = 'sigmoid', padding = 'same')(layer6)

	#Finalize the model
	model = Model(inputs=[input], outputs=[output])
	return model

def Cardiac_Net(block_type, merge_type, dim = (160, 160, 1), filters = 16):
	#General parameter choice
	block = {'Dense': DenseBlock, 'Residual': ResidualBlock,  'Inception': InceptionBlock, "SqueezeExcitation": SqueezeExcitatioBlock, "Convolution": ConvolutionBlock}
	block = block[block_type]
	...
	output = Conv2D(1, (1, 1), activation = 'sigmoid', padding = 'same')(layer4)
